...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    float bytes_transferred = 2048 * 4; // 4 bytes per read
    float clocks = stop_time - start_time;
    float time_spent = clocks / 125000000;
    float throughput = (bytes_transferred / 1000000) / time_spent; 
    printf("You transferred %.0f bytes of data in %f seconds\n", bytes_transferred, time_spent);
    printf("Measured Transfer throughput = %.2f Mbytes/sec\n", throughput);
}
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 6 Kira Emmons - Custom Peripheral Demonstration\n\r");
